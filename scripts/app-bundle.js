...
define('resources/elements/data-table/data-table',['exports', 'aurelia-framework', '../../../models', 'aurelia-dialog'], function (exports, _aureliaFramework, _models, _aureliaDialog) {
  ...
  var DataTable = exports.DataTable = (_dec = (0, _aureliaFramework.inject)(_aureliaDialog.DialogService), _dec(_class = (_class2 = function () {
    ...
    DataTable.prototype.loadItems = function loadItems() {
      ...
      this.service.loadItems(index, this.rows).then(function (res) {
        console.log(res);
        _this.loading = false;
      });
    };
    ...
    DataTable.prototype.applyFilters = function applyFilters() {
      ...
      this.dialog.open({ viewModel: this.filterModal, model: { defaultFilters: this.config.filters, currentFilters: this.lastFilters } }).whenClosed(function (res) {
        console.log(res);
        if (!res.wasCancelled) {
          _this3.lastFilters = res.output;
          _this3.service.loadItems(0, _this3.rows, res.output).then(function (response) {
            if (response.success) {
              _this3.loadItems();
            }
          });
...
define('text!resources/elements/modals/events-filter.html', ['module'], function(module) { module.exports = "<template><require from=material-datetime-picker/material-datetime-picker.css></require><div class=\"mdl-card mdl-shadow--4dp\"><div class=mdl-card__title><h2 class=mdl-card__title-text>Filters</h2></div><div class=mdl-card__supporting-text><div class=mdl-grid><div class=\"mdl-cell mdl-cell--6-col mdl-cell--12-col-tablet\"><compose repeat.for=\"filter of model.currentFilters.filtersLeft\" view-model=../filters/filter-${filter.type} model.two-way=filter></compose></div><div class=\"mdl-cell mdl-cell--6-col mdl-cell--12-col-tablet\"><compose repeat.for=\"filter of model.currentFilters.filtersRight\" view-model=../filters/filter-${filter.type} model.two-way=filter></compose></div><div class=\"mdl-cell mdl-cell--12-col\"><compose repeat.for=\"filter of model.currentFilters.filtersBottom\" view-model=../filters/filter-${filter.type} model.two-way=filter></compose></div></div><div class=\"mdl-card__actions text-right\"><button type=button class=\"mdl-button mdl-js-button left\" click.trigger=clear()>Clear</button><button type=button class=\"mdl-button mdl-js-button\" click.trigger=controller.cancel()>Cancel</button><button type=button class=\"mdl-button mdl-js-button mdl-button--primary\" click.trigger=\"controller.ok({filtersBottom:filtersBottom, filtersLeft:filtersLeft, filtersRight: filtersRight})\">Ok</button></div></div></div></template>"; });
